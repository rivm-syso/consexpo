...
{
    /// <summary>
    /// In this model, all of the substance is released at once from the packaging and transferred to the contents.
    /// </summary>
    internal class OralExposureMigrationFromPackagingInstantRelease : OralExposureBase, IOralExposureSubmodel
    {
        private const OralExposureSubmodelTypes type = OralExposureSubmodelTypes.MigrationFromPackagingInstantRelease;

        public OralExposureSubmodelTypes Type => type;

        public OralExposureMigrationFromPackagingInstantRelease(ScenarioModel scenario)
            : base(scenario, type)
        {
        ...
        private Dose InputToExposure()
        {
            ...
            Amigr = Atot; // all released at once
        ...
        public IEnumerable<ValidationResult> Validate()
        {
            ...
            if (route.IngestedAmountPackaging.HasValue && route.PackagedAmount.HasValue)
            {
                if (route.IngestedAmountPackaging.InGram() > route.PackagedAmount.InGram())
                {
                    ...
                    validationResults.Add(new ValidationResult(string.Format("The {0} must not be more than the {1}", ingestedAmountDisplayName, packagedAmountDisplayName)));
                }
            }
